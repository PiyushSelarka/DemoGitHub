...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    var arrHtmls = [MDLHtml]()
    private var webCellHeights: [CGFloat] = [0]
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "TblCell") as? TblCell {

            cell.delegete = self
            cell.loadWebView(arrHtmls[indexPath.row].html, baseURL: URL.init(string: arrHtmls[indexPath.row].baseUrl)!)
            return cell
        }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if webCellHeights[indexPath.row] != 0 {
...
extension ViewController: updateHeightDelegete {
    func updateHeight(height:CGFloat, cell: UITableViewCell) {
        guard let indexPath = tblView.indexPath(for: cell) else { return }
        webCellHeights[indexPath.row] = height
        DispatchQueue.main.asyncAfter(deadline: (.now() + (0.2*Double(indexPath.row)))) {
